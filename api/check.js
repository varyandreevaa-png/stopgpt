...
{"recommendations":["рекомендация 1","рекомендация 2","рекомендация 3","рекомендация 4"]}`;
  ...
  // ── Основной анализ ────────────────────────────────────────────────────────
  // Для БЕСПЛАТНЫХ проверок: метрики + подсветка фрагментов, без детального анализа.
  // Для ПЛАТНЫХ: полный анализ с сегментами, деталями, вердиктом.

  const freePrompt = `Ты — система анализа текста на признаки ИИ-генерации.
...
Выдели 2-4 фрагмента из текста и пометь каждый: "ai" (признак нейросети) или "original" (живой текст).
Верни ТОЛЬКО JSON:
{"originality":<0-100>,"ai_probability":<0-100>,"readability":<0-100>,"verdict":"<3-4 слова>","orig_desc":"<5-6 слов>","ai_desc":"<5-6 слов>","read_desc":"<5-6 слов>","segments":[{"text":"<точный фрагмент из текста>","type":"ai|original","reason":"<5-8 слов>"}]}`;
...
- originality + ai_probability не обязаны давать 100 (это независимые метрики)`;
  ...
  const maxTokens = isPaid ? 2500 : 500;
  ...
  try {
    ...
    // Для бесплатных — показываем подсветку, но убираем детальный анализ
    if (!isPaid) {
      parsed.details = [];
      parsed._free = true;
    }
